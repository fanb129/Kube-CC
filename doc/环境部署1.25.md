...
2. 通过源码进行部署操作。进入$/kube-prometheus/$目录执行下面三条命令：
   ...
   此处支持Pod运行的镜像我已经全部放到了我的阿里云镜像仓库（见下文），如果存在有Pod无法运行，拉取不到镜像的情况。手动修改对应yaml文件进行拉取。（具体操作后面会补充）
...
[容器镜像服务 (aliyun.com)](https://cr.console.aliyun.com/cn-chengdu/instance/repositories)
