...
type User struct {
	...
	Cpu        string `gorm:"type:varchar(20);not null"` //Cpu配额
	Memory     string `gorm:"type:varchar(20);not null"` //内存配额
	Storage    string `gorm:"type:varchar(20);not null"` //存储配额
	Pvcstorage string `gorm:"type:varchar(20);not null"` //持久化存储配额
	Gpu        string `gorm:"type:varchar(20);not null"` //Gpu配额
}
